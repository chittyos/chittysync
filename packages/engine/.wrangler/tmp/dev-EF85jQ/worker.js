...
// .wrangler/tmp/bundle-5Dq8JH/strip-cf-connecting-ip-header.js
...
var to, Ce, ro, no, io, so, oo, a, z, I, ie, An, Te, N, _, In, Pn, $n, S, x, v, g, y, m, p, we, je, $o, He, ni, O, $e, ii, qt, Qt, jt, Ht, ci, li, di, mi, Ei, Ai, Bi, Ri, Je, Xe, et, qi, tr, rr, nr, ir, sr, uu, or, Qi, ur, ar, Wi, $i, zi, Zi, mt, Xi, Tu, es, ts, pr, ns, gt, cs, ps, ys, ds, Mu, E, _e, wt, Yr, ms, ws, bs, vs, an, Es, _s, hn, Bs, Ms, Ds, Cc, ks, Us, qs, Hs, bn, Ct, Tt, Ks, zs, xn, Ae, $s, qc, Js, Qe, En, vn, _n, export_ClientBase, export_Connection, export_DatabaseError, export_Query, export_defaults, export_types;
var init_serverless = __esm({
  "../../node_modules/.pnpm/@neondatabase+serverless@0.9.5/node_modules/@neondatabase/serverless/index.mjs"() {
    ...
    oo = /* @__PURE__ */ __name((r, e, t) => e in r ? Ce(r, e, { enumerable: true, configurable: true, writable: true, value: t }) : r[e] = t, "oo");
    ...
    An = /* @__PURE__ */ __name((r, e, t, n) => {
      if (e && typeof e == "object" || typeof e == "function")
        for (let i of no(e))
          !so.call(r, i) && i !== t && Ce(r, i, { get: () => e[i], enumerable: !(n = ro(e, i)) || n.enumerable });
      return r;
    }, "An");
    Te = /* @__PURE__ */ __name((r, e, t) => (t = r != null ? to(io(r)) : {}, An(e || !r || !r.__esModule ? Ce(t, "default", {
      value: r,
      enumerable: true
    }) : t, r)), "Te");
    N = /* @__PURE__ */ __name((r) => An(Ce({}, "__esModule", { value: true }), r), "N");
    _ = /* @__PURE__ */ __name((r, e, t) => oo(r, typeof e != "symbol" ? e + "" : e, t), "_");
    In = I((nt) => {
      "use strict";
      p();
      nt.byteLength = uo;
...
`);
    ...
    Ks = Te(et());
    zs = Te(mt());
    xn = /* @__PURE__ */ __name(class xn2 extends Error {
      constructor() {
        super(...arguments);
    ...
    Js = Te(gt());
    Qe = Te(Ct());
    En = /* @__PURE__ */ __name(class En2 extends Tt.Client {
      constructor(t) {
        super(t);
        this.config = t;
      }
    ...
    __name(Wc, "Wc");
    a(Wc, "promisify");
    _n = /* @__PURE__ */ __name(class _n2 extends Tt.Pool {
      constructor() {
        super(...arguments);
        _(this, "Client", vn);
...
// src/db/neon-worker.ts
var neon_worker_exports = {};
__export(neon_worker_exports, {
  getDb: () => getDb,
  query: () => query,
  rawQuery: () => rawQuery
});
function getDb(databaseUrl) {
  ...
  if (!sql) {
    const baseSql = Ys(databaseUrl);
    baseSql.unsafe = (rawSql) => ({ __raw: rawSql });
    sql = baseSql;
    sqlCache.set(databaseUrl, sql);
  }
  return sql;
}
async function query(databaseUrl, queryText, params = []) {
  const sql = getDb(databaseUrl);
  const result = await sql(queryText, params);
  return result;
}
async function rawQuery(databaseUrl, queryText) {
  const sql = getDb(databaseUrl);
  const result = await sql(queryText);
  return result;
}
var sqlCache;
var init_neon_worker = __esm({
  "src/db/neon-worker.ts"() {
    "use strict";
    init_strip_cf_connecting_ip_header();
    init_modules_watch_stub();
    ...
    init_performance2();
    init_serverless();
    _e.fetchConnectionCache = true;
    sqlCache = /* @__PURE__ */ new Map();
    __name(getDb, "getDb");
    __name(query, "query");
    __name(rawQuery, "rawQuery");
  }
});

// .wrangler/tmp/bundle-5Dq8JH/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
// .wrangler/tmp/bundle-5Dq8JH/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
...
var NotionAdapter = class {
  apiToken;
  constructor(config2) {
    this.apiToken = config2.apiToken;
  }
  async request(method, path, body) {
    ...
    if (!res.ok) {
      const error3 = await res.json().catch(() => ({ message: res.statusText }));
      throw new Error(`Notion API error: ${error3.message || res.status}`);
    }
    return res.json();
  }
  /**
  ...
  extractPropertyValue(prop) {
    switch (prop.type) {
      ...
      case "people":
        return prop.people?.map((p2) => p2.id) || [];
      case "files":
   ...
   */
  formatPropertyValue(name, value) {
    ...
    if (Array.isArray(value)) {
      if (value.every((v2) => typeof v2 === "string")) {
        return { multi_select: value.map((v2) => ({ name: v2 })) };
      }
      if (value.every((v2) => typeof v2 === "string" && v2.includes("-"))) {
  ...
  formatTypedValue(type, value) {
    switch (type) {
      ...
      case "multi_select":
        return {
          multi_select: (Array.isArray(value) ? value : [value]).map((v2) => ({
            name: String(v2)
          }))
        };
      case "status":
...
__name(NotionAdapter, "NotionAdapter");
...
var SyncEngine = class {
    ...
    try {
      ...
      if (!options.dryRun) {
        ...
        for (const change of resolvedChanges) {
          try {
            await this.applyChange(change, config2);
        ...
        await this.saveSnapshot(config2.id, sourceData, targetData);
      } else {
      ...
      await this.writeAuditLog(syncId, config2, result);
   ...
   */
  async fetchNotionData(databaseId) {
   ...
   */
  async fetchPostgresData(tableName) {
    const { rawQuery: rawQuery2 } = await Promise.resolve().then(() => (init_neon_worker(), neon_worker_exports));
    const safeTableName = tableName.replace(/[^a-zA-Z0-9_]/g, "");
    const query2 = `SELECT * FROM ${safeTableName} ORDER BY updated_at DESC NULLS LAST`;
    const rows = await rawQuery2(this.databaseUrl, query2);
    return rows.map((row) => ({
   ...
   */
  calculateDiffs(sourceData, targetData, lastSnapshot, fieldMapping) {
    const sourceChanges = [];
    const targetChanges = [];
   ...
   */
  resolveConflicts(sourceChanges, targetChanges, strategy) {
    const conflicts = [];
   ...
   */
  async applyChange(change, config2) {
    const target = change.direction === "source_to_target" ? config2.target : config2.source;
    switch (target.type) {
      case "notion":
   ...
   */
  async applyNotionChange(change, databaseId) {
    if (!this.notionAdapter) {
      throw new Error("Notion adapter not configured");
    }
    switch (change.type) {
   ...
   */
  async applyPostgresChange(change, tableName) {
    const { rawQuery: rawQuery2 } = await Promise.resolve().then(() => (init_neon_worker(), neon_worker_exports));
    const safeTableName = tableName.replace(/[^a-zA-Z0-9_]/g, "");
    switch (change.type) {
      case "create": {
        ...
        const values = Object.values(data).map((v2) => typeof v2 === "string" ? `'${v2.replace(/'/g, "''")}'` : v2).join(", ");
        await rawQuery2(this.databaseUrl, `INSERT INTO ${safeTableName} (${columns}) VALUES (${values})`);
        break;
      }
      case "update": {
        const data = change.after || {};
        const setClause = Object.entries(data).map(([k, v2]) => {
          const safeKey = k.replace(/[^a-zA-Z0-9_]/g, "");
          const safeVal = typeof v2 === "string" ? `'${v2.replace(/'/g, "''")}'` : v2;
          return `${safeKey} = ${safeVal}`;
        }).join(", ");
        const safeRowId = String(change.rowId).replace(/'/g, "''");
        await rawQuery2(this.databaseUrl, `UPDATE ${safeTableName} SET ${setClause} WHERE id = '${safeRowId}'`);
        break;
      }
      case "delete": {
        const safeRowId = String(change.rowId).replace(/'/g, "''");
        await rawQuery2(this.databaseUrl, `DELETE FROM ${safeTableName} WHERE id = '${safeRowId}'`);
        break;
      }
    }
  }
   ...
   */
  async applySheetsChange(change, sheetUrl) {
    throw new Error("Google Sheets integration not yet implemented");
  }
  /**
   * Validate changes against schema
   */
  async validateChanges(changes, schemaId) {
    if (!this.schemaUrl)
      return [];
    const errors = [];
    try {
   ...
   */
  async getLastSnapshot(configId) {
    const { query: query2 } = await Promise.resolve().then(() => (init_neon_worker(), neon_worker_exports));
    try {
   ...
   */
  async saveSnapshot(configId, sourceData, targetData) {
   ...
   */
  async writeAuditLog(syncId, config2, result) {
    const sql = getDb(this.databaseUrl);
    await sql`
      ...
      VALUES (
        ${syncId}, ${config2.id}, ${result.status},
   ...
   */
  mapFields(data, mapping, direction) {
    const result = {};
    for (const map of mapping) {
      if (map.readonly && direction === "target_to_source")
        continue;
   ...
   */
  getChangedFields(record1, record2, mapping) {
    const changed = [];
    for (const map of mapping) {
...
syncRoutes.post("/execute", async (c) => {
  ...
  const result = await engine.sync(config2, { dryRun: body.dryRun });
...
syncRoutes.post("/dry-run", async (c) => {
  ...
  const result = await engine.sync(config2, { dryRun: true });
  return c.json({
    preview: true,
    ...result
  });
...
syncRoutes.post("/validate", async (c) => {
  ...
  if (!config2.fieldMapping || config2.fieldMapping.length === 0) {
    errors.push("At least one field mapping is required");
  }
  if (config2.source) {
    if (!["notion", "postgres", "sheets"].includes(config2.source.type)) {
      errors.push(`Invalid source type: ${config2.source.type}`);
    }
    if (!config2.source.id) {
      errors.push("source.id is required");
    }
  }
  if (config2.target) {
    if (!["notion", "postgres", "sheets"].includes(config2.target.type)) {
      errors.push(`Invalid target type: ${config2.target.type}`);
    }
    if (!config2.target.id) {
      errors.push("target.id is required");
    }
  }
  if (config2.mode && !["bidirectional", "source_to_target", "target_to_source"].includes(config2.mode)) {
    errors.push(`Invalid mode: ${config2.mode}`);
  }
  if (config2.conflictResolution && !["last_write_wins", "source_wins", "target_wins", "manual"].includes(config2.conflictResolution)) {
    errors.push(`Invalid conflictResolution: ${config2.conflictResolution}`);
  }
  if (errors.length > 0) {
...
healthRoutes.get("/", async (c) => {
  ...
  const overallStatus = Object.values(checks).every(
    (c2) => c2.status === "healthy" || c2.status === "not_configured" || c2.status === "degraded"
  ) ? "healthy" : "unhealthy";
  return c.json({
...
registryRoutes.post("/", async (c) => {
  const config2 = await c.req.json();
  const sql = getDb(c.env.DATABASE_URL);
  const id = config2.id || crypto.randomUUID();
  const fullConfig = {
    id,
    name: config2.name || "Untitled Registry",
    source: config2.source,
    target: config2.target,
    mode: config2.mode || "bidirectional",
    conflictResolution: config2.conflictResolution || "last_write_wins",
    fieldMapping: config2.fieldMapping || [],
...
registryRoutes.put("/:id", async (c) => {
  ...
  const updated = {
    ...
    schedule: updates.schedule !== void 0 ? updates.schedule : current.schedule,
    enabled: updates.enabled !== void 0 ? updates.enabled : current.enabled
  };
  await sql`
    UPDATE sync_configs
    SET
...
auditRoutes.get("/", async (c) => {
  ...
  const query2 = `
    ...
    OFFSET ${parseInt(offset)}
  `;
  const rows = await rawQuery(c.env.DATABASE_URL, query2);
  const countQuery = `
    SELECT COUNT(*) as total
    FROM sync_audit_log
    ${whereClause}
  `;
  const countResult = await rawQuery(c.env.DATABASE_URL, countQuery);
  return c.json({
    entries: rows,
...
auditRoutes.get("/:syncId", async (c) => {
  const syncId = c.req.param("syncId");
  const query2 = `SELECT * FROM sync_audit_log WHERE sync_id = '${syncId.replace(/'/g, "''")}'`;
  const rows = await rawQuery(c.env.DATABASE_URL, query2);
  if (rows.length === 0) {
  ...
  if (format === "csv") {
    ...
    return new Response(csvRows.join("\n"), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="audit-export-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv"`
      }
    });
  }
  return c.json({
...
auditRoutes.get("/summary", async (c) => {
  ...
  `;
  const summary = await rawQuery(c.env.DATABASE_URL, summaryQuery);
  const dailyQuery = `
    SELECT
...
app.get("/api/v1/status", async (c) => {
  ...
  let dbHealthy = false;
  try {
    const { getDb: getDb2 } = await Promise.resolve().then(() => (init_neon_worker(), neon_worker_exports));
    const sql = getDb2(c.env.DATABASE_URL);
    await sql`SELECT 1`;
    dbHealthy = true;
  } catch {
    dbHealthy = false;
